...
// ═══════════════════════════════════════════════════════════════════════════
//  HISTORY PANEL — zid f akhir main.js
// ═══════════════════════════════════════════════════════════════════════════

function openHistoryPanel(clientId, clientName) {
  let panel   = document.getElementById('history-panel');
  let overlay = document.getElementById('history-panel-overlay');

  // Incha panel f DOM ida mabanixi
  if (!panel) {
    ...
    panel.innerHTML = `
      <div class="history-panel-header">
        <span class="history-panel-title" id="history-panel-title">Client History</span>
        <button class="history-panel-close" onclick="closeHistoryPanel()">✕</button>
      </div>
  ...
  document.getElementById('history-panel-title').textContent = clientName + ' — History';
  document.getElementById('history-panel-body').innerHTML =
    '<p class="muted" style="font-size:.85rem;padding:12px 0">Loading…</p>';

  overlay.classList.add('open');
  panel.classList.add('open');
  document.body.style.overflow = 'hidden';

  fetch('/barber/client-history/' + clientId)
    .then(r => r.json())
    .then(data => {
      const body = document.getElementById('history-panel-body');
      if (!data.length) {
        body.innerHTML = '<div class="empty-state"><p>No visits yet.</p></div>';
        return;
      }
      body.innerHTML = data.map(v => `
        <div class="history-visit-row">
          <div class="history-visit-date">📅 ${v.appointment_time || v.created_at}</div>
          <div class="history-visit-services">${escHtml(v.services || '—')}</div>
          <div class="history-visit-price">${parseFloat(v.total_price || 0).toFixed(2)} MAD</div>
          ${v.status ? `<span class="badge badge-${v.status}" style="margin-top:4px">${v.status}</span>` : ''}
        </div>`).join('');
    })
    .catch(() => {
      const body = document.getElementById('history-panel-body');
      body.innerHTML = '<p class="muted" style="font-size:.85rem;color:var(--red)">Failed to load history.</p>';
    });
}
